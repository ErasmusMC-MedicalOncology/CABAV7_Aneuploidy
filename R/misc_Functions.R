...
plotKM.Treatment <- function(fit, ylim, palette = 'jco', hr = NULL){
    ...
    p.goodvsbad <- survminer::pairwise_survdiff(fit$call$formula, data = fit$call$data)
    if(all(dim(p.goodvsbad$p.value) == c(2,2))){
        p.goodvsbad <- p.goodvsbad$p.value[2,2]
        if(p.goodvsbad > 0.1) p.goodvsbad <- round(p.goodvsbad, 2)
        if(p.goodvsbad < 0.05) p.goodvsbad <- round(p.goodvsbad, 3)
        x$plot <- x$plot + ggplot2::annotate('text', x = 1500, y = .8, size = 2.5, label = paste0('Bad vs. Good Responders: log-rank q = ', p.goodvsbad), family='Roboto')
    }
